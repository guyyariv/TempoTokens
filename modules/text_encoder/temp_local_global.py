...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if not locals is None and indices[0].shape[0]:
                inputs_embeds[indices] = locals[0].view(temp_token.shape[0]*temp_token.shape[1], 1024)

            indices = torch.where(input_ids == 49411)
            if not locals is None and indices[0].shape[0]:
                inputs_embeds[indices] = locals[1].view(temp_token.shape[0]*temp_token.shape[1], 1024)

            indices = torch.where(input_ids == 49412)
            if not locals is None and indices[0].shape[0]:
                inputs_embeds[indices] = locals[2].view(temp_token.shape[0]*temp_token.shape[1], 1024)
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.embeddings(input_ids=input_ids, position_ids=position_ids, audio_token=audio_token, temp_token=temp_token, locals=locals)
