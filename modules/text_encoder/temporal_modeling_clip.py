...
class CLIPEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.layer_idxs = [0, 5, 10, 15, 20]
        self.temp_layers = nn.ModuleList([CLIPEncoderLayer(config) for _ in range(len(self.layer_idxs))])
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.layer_idxs and hidden_states.shape[0] > 1:
                num_frames = 24
                bs = hidden_states.shape[0] // num_frames
                hidden_states = hidden_states.reshape((bs*77, num_frames, -1))
                hidden_states = self.temp_layers[self.layer_idxs.index(idx)](
                    hidden_states,
                    attention_mask,
                    None,
                    output_attentions=output_attentions,
                )[0]
                hidden_states = hidden_states.reshape((bs * num_frames, 77, -1))
